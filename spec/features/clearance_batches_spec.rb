...
RSpec.describe "add new monthly clearance_batch" do

  describe "clearance_batches index", type: :feature do

    context "when previous clearance batches exist" do
      ...
      context "when clearance batch includes items" do
        before do
          FactoryBot.create_list(:item, 5, clearance_batch: clearance_batch_1, status: "clearanced", price_sold: 5)
        end

        it "shows items in a clearance batch" do
          visit root_path
          click_link "Clearance Batch #{clearance_batch_1.id}"

          aggregate_failures do
            expect(page).to have_content("Clearance Batch #{clearance_batch_1.id}")
            expect(page).to have_content("Total items: 5")
